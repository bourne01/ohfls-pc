	<template>
		...
		<el-table    
			...
			min-width="170">
			<template slot-scope="scope"> 
				<actions :current-course="scope.row" @update-course-list="getCourseList"></actions>
			</template>      
			</el-table-column>                        <!--操作列-->
			</el-table>
			<div class="footer">
	...
	export default {
		...
		methods: {
		...mapMutations('election',['SET_CURRENT_COURSE_IDS']),   
		/**@function 提交选课计划*/
		submit(){
			...
			this['SET_CURRENT_COURSE_IDS'](courseIdList);
		...
		handleByBatch(type){
			let params = {};
			if(!this.courseIds){
				this.$message.warning('请先选择要操作的课程！');
				return;
			}
